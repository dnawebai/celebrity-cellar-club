...
-- These are called only by the service role (server functions, webhooks, cron) or by triggers,
-- which run with the trigger owner's rights and are unaffected by EXECUTE grants.
...
REVOKE EXECUTE ON FUNCTION public.validate_profile_age() FROM PUBLIC, anon, authenticated;
